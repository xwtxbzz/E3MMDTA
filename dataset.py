...
class DTAData(InMemoryDataset):
    def __init__(self,aff,dataset_name = 'davis'):
        self.seq = pd.read_csv("/workspace/dataset/private/xyj/data/{}/sequence.csv".format(dataset_name))
        self.smi = pd.read_csv("/workspace/dataset/private/xyj/data/{}/smiles.csv".format(dataset_name))
    ...
    def __getitem__(self, index):
        ...
        seq =  self.seq[self.seq['TARGETID']==self.aff[index][1]]
        ...
        d_features = np.load('/workspace/dataset/private/xyj/data/{}/{}/{}.npz'.format(self.dataset,'drug_graph', drug_id), allow_pickle=True)
        smiles_size, smiles_features, smiles_edge_index = d_features['smiles_features'].shape[0], d_features['smiles_features'], d_features['smiles_edge_index']
        t_features = np.load('/workspace/dataset/private/xyj/data/{}/{}/{}.npz'.format(self.dataset,'protein_graph', target_id), allow_pickle=True)
        target_size, target_features, target_edge_index = t_features['target_features'].shape[0], t_features['target_features'], t_features['target_edge_index']
        smile_graph = Data(x=torch.Tensor(smiles_features), 
                           edge_index=torch.LongTensor(smiles_edge_index).transpose(1,0),
                           y=value)
        target_graph = Data(x=torch.Tensor(target_features), 
        ...
        finger = np.load('/workspace/dataset/private/xyj/data/{}/{}/{}.npz'.format(self.dataset,'drug_fingerprints', drug_id), allow_pickle=True)
